...
public class GamePanel extends JPanel implements Runnable { 
    ...
    // SYSTEM
    public TileManager tileM = new TileManager(this); 		// 匯入背景
    ...
    public final int maxProjectTile = 20;
    ...
    public Entity[][] projectileList = new Entity[maxMap][maxProjectTile];		// 存放 Project tile
...
*/
	...
	public void run() {
		...
		double drawInterval = 1_000_000_000/FPS;	// 計算每次繪製之間的時間間隔（以奈秒為單位）
...
//				drawcount++;					// 增加繪製幀數計數器。
			...
			if(timer >= 1_000_000_000) {			// 檢查 timer 是否大於或等於 1 秒（1000000000 奈秒）。
			...
			while (iterator.hasNext()) {
			    ...
			    if (particle != null) {
			        if (particle.alive) {
			            ...
			            iterator.remove(); // 使用迭代器的 remove() 方法安全地移除
			        }
			    }
			}
