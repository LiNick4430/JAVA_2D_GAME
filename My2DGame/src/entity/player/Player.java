...
public class Player extends Entity {
	...
	public void getPlayerAttackImage() {
		...
		}else if (currentWeapon.type == Type.AXE) {
	...
	public void update() {
		...
		if (gp.keyH.shotKeyPressed == true && projectile.alive == false 
			...
			// CHECK VACANCY
			for (int i = 0; i < gp.projectileList[gp.currentMap].length; i++) {
	...
	public void pickUpObject(int i) {
		...
		if (i != 999) {
			...
			// PICK ONLY ITEMS
			if (gp.obj[gp.currentMap][i].type == Type.PICKUP_ONLY) {
				
				gp.obj[gp.currentMap][i].use(this);
				gp.obj[gp.currentMap][i] = null;
			}
			// OBSTACLE
			else if (gp.obj[gp.currentMap][i].type == Type.OBSTACLE) {
	...
	public void selectItem() {
		...
		if (itemIndex < inventory.size()) {
			...
			if (selectItem.type == Type.SWORD || selectItem.type == Type.AXE) {
			...
			if (selectItem.type == Type.SHIED) {	
				
				currentShield = selectItem;
				defense = getDefense();
			}
			
			if (selectItem.type == Type.CONSUMABLE) {
